...
class MainActivity : AppCompatActivity() {
    ...
    private fun initializeProfile() {
        ...
        Picasso.get().load(imageProfile).transform(RoundedCornersTransformation(10f))
            .into(profileImage)
    }
        ...
        mapFragment.getMapAsync { map ->
            ...
            map.animateCamera(
                CameraUpdateFactory.newLatLngZoom(
                    LatLng(48.20850932439838, 41.25624814967147),
                    6.4f
                )
            )
                    ...
                    db?.collection("reports")?.document(idOfPoint)?.get()
                        ?.addOnSuccessListener { res ->
                            ...
                            storageRef.downloadUrl.addOnSuccessListener { uri ->
                                Picasso.get().load(uri).transform(RoundedCornersTransformation(20f))
                                    .into(imgoftask)
                            }.addOnFailureListener { exception ->
                                Log.e(
                                    "20241",
                                    "Не получилось загрузить изображение у picaso: ${exception.message}"
                                )
                            }
    ...
    private fun openFAQquestions() {
        val url =
            "https://xn--80aaeafhdblwf1cagbb6blqcb6r.xn--p1ai/%d1%87%d0%b0%d1%81%d1%82%d0%be-%d0%b7%d0%b0%d0%b4%d0%b0%d0%b2%d0%b0%d0%b5%d0%bc%d1%8b%d0%b5-%d0%b2%d0%be%d0%bf%d1%80%d0%be%d1%81%d1%8b/"
    ...
    private fun switchView(
        toShow: ConstraintLayout,
        toHide1: ConstraintLayout,
        toHide2: ConstraintLayout,
        direction: Boolean
    ) {
        ...
        val fadeOut1 =
            ObjectAnimator.ofFloat(toHide1, "alpha", 1f, 0f).apply { duration = duration2 }
        val fadeOut2 =
            ObjectAnimator.ofFloat(toHide2, "alpha", 1f, 0f).apply { duration = duration2 }
        val slideOut1 = ObjectAnimator.ofFloat(
            toHide1,
            "translationX",
            0f,
            -toHide1.width.toFloat() * slideDirection
        ).apply { duration = duration2 }
        val slideOut2 = ObjectAnimator.ofFloat(
            toHide2,
            "translationX",
            0f,
            -toHide2.width.toFloat() * slideDirection
        ).apply { duration = duration2 }
        ...
        val slideIn = ObjectAnimator.ofFloat(
            toShow,
            "translationX",
            toShow.width.toFloat() * slideDirection,
            0f
        ).apply { duration = duration2 }
    ...
    fun limitStringLength(input: String): String {
        return "${input.take(30)}..."
    }
}